...
function get_average_rms_from_selected_items()
  average_RMS = {} -- RMS values are collected to this table
  for i=1, reaper.CountSelectedMediaItems(0) do
    ...
    if item ~= nil then
      ...
      if take ~= nil and not reaper.TakeIsMIDI(take) then
        -- Function desc:
        -- get_average_rms(MediaItem_Take, bool adj_for_take_vol, bool adj_for_item_vol, bool adj_for_take_pan, bool val_is_dB)
        RMS_t = get_average_rms(take, true, true, true, true)
        average_RMS[#average_RMS+1] = RMS_t
        msg("Item " .. i .. " Average RMS")
        for j=1, #RMS_t do
          msg("Channel " .. j .. ": " .. RMS_t[j] .. " dB")
        end
        msg("")
      end
    end
...
-- ("defer" is used here to prevent the undo point creation)
reaper.defer(get_average_rms_from_selected_items)
